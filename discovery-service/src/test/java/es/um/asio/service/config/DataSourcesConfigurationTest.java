...
class DataSourcesConfigurationTest {
    ...
    void setUp() {
        ...
        for (int i = 1 ; i <=5; i++) {
            ...
            for (int j = 1 ; j <=2; j++) {
                DataSourcesConfiguration.Node.TripleStore ts =new DataSourcesConfiguration.Node.TripleStore(String.format("type_%s",j),String.format("baseURL_%s",j),String.format("user_%s",j),String.format("password_%s",j));
                tripleStores.add(ts);
            }
